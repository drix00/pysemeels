...
class TestGenerateHdf5File(unittest.TestCase):
    ...
    def test_add_spectrum(self):
        ...
        with h5py.File(self.hdf5_file_path, 'w') as hdf5_file:
            ...
            self.assertEqual(0.0, spectrum_group.attrs[HDF5_SAMPLE_HEIGHT])
            ...
            self.assertEqual(0.0, spectrum_group.attrs[HDF5_ENERGY_LOSS])
            self.assertEqual(500, spectrum_group.attrs[HDF5_ACQUISITION_SPEED])

            self.assertEqual("01/Mar/2017",spectrum_group.attrs[HDF5_DATE])
            self.assertEqual("10:59", spectrum_group.attrs[HDF5_TIME])
            self.assertEqual("", spectrum_group.attrs[HDF5_COMMENT])
            self.assertEqual(500.0, spectrum_group.attrs[HDF5_ACQUISITION_SPEED])
            self.assertEqual(0.0, spectrum_group.attrs[HDF5_ENERGY_LOSS])
            self.assertEqual(98.7, spectrum_group.attrs[HDF5_RAW])
            self.assertEqual(7.0, spectrum_group.attrs[HDF5_ENERGY_WIDTH_eV])
            self.assertEqual(586, spectrum_group.attrs[HDF5_DUAL_DET_POSITION])
            self.assertEqual(133, spectrum_group.attrs[HDF5_DUAL_DET_POST])
            self.assertEqual(608, spectrum_group.attrs[HDF5_DUAL_DET_CENTER])
